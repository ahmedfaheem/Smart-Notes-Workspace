...
export default function ManipulateNote({ mode }) {

  const navigate = useNavigate();
  const isEditMode = mode === "edit";
  const id = isEditMode ? useParams().id : null;
  const token = useSelector((state) => state.auth.token);
  const [saved, setSaved] = useState(false);

  const {
    data: noteData,
    isLoading: isloadingNoteData,
    isError: isFetchingNoteError,
    error: fetchingNoteError,
  } = useQuery({
    queryKey: ["note", id],
    queryFn: () => getNote(token, id),
    enabled: !!id && !!token && !!isEditMode, // Only fetch if id is available
  });
  ...
  const {
    ...
    mutationFn: (data) => {
      return isEditMode ? updateNote(token, id, data) : createNote(token, data);
    },
    onSuccess: () => {
      setSaved(true);
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="relative">
        ...
        {isSaving || isloadingNoteData && (
          <div className="absolute inset-0 z-50 flex items-center justify-center rounded-3xl bg-black/20 backdrop-blur-sm">
            <div className="rounded-xl bg-white dark:bg-zinc-900 px-6 py-5 shadow-xl flex items-center gap-3">
              <LoaderCircle className="h-6 w-6 animate-spin text-indigo-500" />
              <span className="font-medium">
                {isloadingNoteData ? "Loading note..." : "Saving note..."}
                </span>
            </div>
          </div>
        )}
